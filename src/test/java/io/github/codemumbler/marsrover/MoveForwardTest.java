...
class MoveForwardTest {
  ...
  public void moveForwardWest() {
    ...
    assertEquals("Vector{direction=W, x=-1, y=0}", moveForward.execute(vector).toString());
  }

  @Test
  public void moveForwardSouth() {
    Vector vector = new Vector(0, 0, Vector.DIRECTION.S);
    assertEquals("Vector{direction=S, x=0, y=-1}", moveForward.execute(vector).toString());
  }
}
